...
module.exports = function (source, mnt, options) {
  ...
  function autoPrefix(pathName, cssString) {
    chatty('autoprefixing', pathName);
    return autoprefixer({ browsers: options.browsers }).process(cssString).css;
  }
  ...
  function getCached(pathName, stat) {
    ...
    var mainIsFresh = stat.mtime.getTime() <= cacheEntry.mtime ? when.resolve() : when.reject();

    var partialsAreFresh = when.all((stat.imports || []).map(function (partial) {
      return node.lift(fs.stat)(partial).then(function (stat) {
        console.log(stat.mtime.getTime(), cacheEntry.mtime, stat.mtime.getTime() > cacheEntry.mtime);
  ...
  function statCompiled(pathName, stat, cb) {
    getCached(pathName, stat).done(node.liftCallback(cb), function () {

      var adapters = extensionMap[path.extname(pathName)];
      var isCss = ['.css', '.scss', '.less', '.myth', '.styl'].indexOf(path.extname(pathName)) !== -1;
      var filePath = path.join(source, pathName);

      if (adapters) {
        chatty('Compiling', adapters[0].engineName, pathName);
        adapters[0].renderFile(filePath).done(function (compiled) {
          chatty('Compiled', adapters[0].engineName, pathName);
          var result = compiled.result;

          if (isCss) {
            result = autoPrefix(pathName, result);
          }

          stat.size = result.length;
        ...
        fs.readFile(filePath, function (err, data) {
          if (err) {
            self.emit('error', err);
            chatty('Error reading file', filePath);
            cb(null, stat);
          }

          var result = autoPrefix(pathName, data);
