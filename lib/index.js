...
module.exports = function (source, mnt) {
  ...
  function getCachedBuffer(pathName) {
    var extension = path.extname(pathName);
    var adapters = extensionMap[extension];

    if (!adapters) {
      console.log('No compiler found:', pathName);
      return new rsvp.Promise(function (resolve, reject) {
        reject();
      });
    } else {
      console.log('Adapter found:', adapters[0].engineName);
      ...
      if (cacheEntry) {
        ...
        if (stat.mtime.getTime() === cacheEntry.mtime) {
          console.log('Cache fresh:', pathName);
          return cacheEntry.promise;
        } else {
          console.log('Cache stale:', pathName);
          self.cache[pathName] = null;
          return getCachedBuffer(pathName);
        }
      } else {
        console.log('Cache miss, compiling:', pathName);
        ...
        fs.stat(pathName, function (err, stat) {
          ...
          self.cache[pathName].mtime = stat.mtime.getTime();
        });

        adapters[0].renderFile(pathName).done(deferred.resolve, deferred.reject);
  ...
  handlers.getattr = function (pathname, cb) {
    fs.stat(path.join(source, pathname), function (err, st) {
      ...
      // If there is a compiler present we need to lie about the size of the file
      // We don't knwo the size before it's compiled
      if (extensionMap[path.extname(pathname)]) {
        st.size = st.size * 10;
      }
  ...
  handlers.read = function (pathname, offset, len, buf, handle, cb) {
    ...
    getCachedBuffer(pathname)
      .then(function (compiled) {
        var length = Math.min(compiled.result.length, len);

        buf.write(compiled.result.slice(offset, length), offset);
