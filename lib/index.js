...
module.exports = function (source, mnt, options) {
  ...
  function chatty() {
    ...
    if (options.verbose) {
      // Append standard prefix
      var out = ['Chatty', new Date().toISOString()];
  ...
  handlers.open = function (pathname, flags, cb) {
    ...
    chatty('opening', sourceFile, 'with flags', flags);
    whenFs.open(sourceFile, flags)
    .tap(function initiateWatches() {
      chatty('initiateWatches');
    ...
    .tap(function initiateTolk() {
      ...
      if (!shouldCompileFile(pathname)) {
        chatty('shouldn\'t compile, returning');
        return;
      }

      if (!self.tolkCache[pathname]) {
        chatty('no cache, creating one');
      ...
      .then(function checkCompileTime(cacheEntry) {
        chatty('checkCompileTime');

        if (!(cacheEntry.compileTime && cacheEntry.tolkPromise)) {
          chatty('no compileTime or no tolkPromise', cacheEntry.compileTime, cacheEntry.tolkPromise);
          return when.reject(cacheEntry);
        } else {
          chatty('have compileTime and tolkPromise, resolving');
          return when.resolve(cacheEntry);
        }
      })
      .then(function compareLastModifiedTime(cacheEntry) {
        chatty('compareLastModifiedTime');

        return cacheEntry.tolkPromise.then(function (compiled) {
          chatty('finished resolving tolkPromise, compiled:', compiled);
