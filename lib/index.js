  ...
  function statCompiled(pathName, stat, cb) {
    getCached(pathName, stat).done(node.liftCallback(cb), function () {
      ...
      if (adapter) {
        ...
        chatty('Compiling', adapter.engineName, pathName, options.accord);
        adapter.renderFile(filePath, options.accord).done(function (compiled) {
          chatty('Compiled', adapter.engineName, pathName);
          ...
          err.message = 'Error compiling ' + pathName + ': ' + err.message;
          self.emit('error', err);
          chatty(err);
          cb(null, stat);
        });
  ...
  fuse.unmount(mnt, function () {
    mkdirp(mnt, function (err) {
      ...
      fuse.mount(mnt, handlers, function () {
        self.emit('adaptersLoaded', loadedAdapters);
        self.emit('mount');
      });
    });
  });
